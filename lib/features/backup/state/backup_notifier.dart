...
class BackupNotifier extends AsyncNotifier<BackupState> {
  ...
  ///     LOCAL — no exception may escape [build()] (EC-07). The probe is
  ///     memoized keyed on `activeProvider` (FR-15) — see [_iCloudProbeKey] /
  ///     [_iCloudProbeMemo]; a `false` result is never memoized (EC-07).
  ///
  ...
  ///
  /// Guard order:
  ///   0. Resolve `future` (FR-21) — makes guards 1/2 non-vacuous during a
  ///      cold-start `AsyncLoading` race.
